...
class UserInfoFormViewController: UIViewController {
    ...
    @IBAction func didTouchContinue(_ sender: Any) {
        if txtuserName.text?.count == 0 || txtFavoriteMovie.text?.count == 0 {
            ...
            perfomAlert.setupViewModel(CustomGenericAlertViewModel(title: "¡Verifica tus datos!", img: "exclamationmark.triangle.fill", overview: "Verifica que todos los campos de texto no esten vacios."), isFromHome: false)
            ...
            guard let textUserName = txtuserName.text, let textFavoriteMovie = txtFavoriteMovie.text else { return }
            UserDefaults.standard.setValue(textUserName, forKey: "userName")
            UserDefaults.standard.setValue(textFavoriteMovie, forKey: "favoriteMovie")
