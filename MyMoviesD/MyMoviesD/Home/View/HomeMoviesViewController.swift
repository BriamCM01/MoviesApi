...
class HomeMoviesViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = .appColor200
        navigationController?.navigationBar.isHidden = true
    
        let hasShowedAlert = UserDefaults.standard.bool(forKey: "hasShowedAlert")
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [self] in
            if !hasShowedAlert {
                let alert = GenericCustomAlertViewController(nibName: "GenericCustomAlertViewController", bundle: .main)
                alert.setupViewModel(CustomGenericAlertViewModel(title: "¡Bienvenido!", img: "play.house.fill", overview: "Espero disfrutes mucho esta app, es algo sencillo y se que se puede mejorar. No soy el mejor en diseño pero espero no lastime tus ojos. Saludos."), isFromHome: true)
...
//        tableViewTopRateMovies.showSkeleton(usingColor: .appColor600, transition: .crossDissolve(0.25))
    ...
    @objc func seePerfil(tapGestureRecognizer: UITapGestureRecognizer) {
        ...
        perfil.modalTransitionStyle = .crossDissolve
        perfil.modalPresentationStyle = .overCurrentContext
...
extension HomeMoviesViewController: UITableViewDelegate, SkeletonTableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        reusableView.modalTransitionStyle = .coverVertical
        reusableView.modalPresentationStyle = .currentContext
